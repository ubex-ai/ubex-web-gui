...
function installDeps() {
	exec('node --version', (err, stdout) => {
		...
		if (err || compareVersions(nodeVersion, '8.10.0') === -1) {
			installDepsCallback(err || `[ERROR] You need Node.js v8.10 or above but you have ${nodeVersion}`);
		} else {
			exec('npm --version', (err2, stdout2) => {
				const npmVersion = stdout2.trim();
				if (err2 || compareVersions(npmVersion, '5.0.0') === -1) {
					installDepsCallback(err2 || `[ERROR] You need npm v5 or above but you have v${npmVersion}`);
				} else {
