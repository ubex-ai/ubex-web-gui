...
class Allcontact extends React.Component {
	render() {
		...
		for (let i = 0; i < this.props.allcontacts.length; i++) {
			allcontactsList.push(
				<ListGroupItem key={i} className="contact">
					<div className="user-img">
						{/*<img*/}
							{/*src={this.props.allcontacts[i].avatar}*/}
							{/*alt="user avatar"*/}
					...
					<div className="user-info">
						...
						<span data-status={this.props.allcontacts[i].status} className="profile-status float-right" />
					</div>
				</ListGroupItem>,
			);
		}
		return (
			<div>
				<ListGroup>{allcontactsList}</ListGroup>
			</div>
		);
	}
}
